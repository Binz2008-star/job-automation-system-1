...
_CONFIRMATION_PHRASES = frozenset([
    ...
    "proceed", "continue", "that works", "sounds good",
])

_NEGATION_PHRASES = frozenset([
    "no", "no thanks", "nope", "don't", "do not", "not", "never",
    "cancel", "stop", "abort", "skip it", "forget it",
])
...
def classify_intent(message: str, *, has_cv_profile: bool = False) -> IntentResult:
    ...
    if lower in _CONFIRMATION_PHRASES:
        return IntentResult("confirmation", 1.0, "exact")

    if lower in _NEGATION_PHRASES:
        return IntentResult("negation", 1.0, "exact")
