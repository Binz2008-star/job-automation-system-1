...
class RicoChatAPI:
    ...
    def _target_role_search_response(self, user_id: str, role: str, profile: Any) -> dict[str, Any]:
        ...
        # Set active search context if no results found
        if not top_matches and role_intelligence_data:
            fallback_roles = [r["role"] for r in role_intelligence_data.get("adjacent_roles", [])]
            self.memory.set_active_search_context(
                user_id=user_id,
            ...
            response["active_search_context"] = {
    ...
    def _handle_active_user(self, user_id: str, message: str) -> dict[str, Any]:
        ...
        Pipeline:
          1. Check conversation state (pending confirmations, active search)
        ...
        # ── Step 0: Check conversation state before intent classification ─────
        intent_result = classify_intent(message, has_cv_profile=has_cv)

        # Pending confirmation: user responded to a confirmation prompt
        if intent_result.intent == "confirmation":
            pending = self.memory.get_pending_confirmation(user_id)
            if pending:
                return self._handle_pending_confirmation(user_id, pending, profile)

        # Negation: user declined a pending action
        if intent_result.intent == "negation":
            pending = self.memory.get_pending_confirmation(user_id)
            if pending:
                self.memory.clear_conversation_state(user_id)
                response = {
                    "type": "confirmation_declined",
                    "message": f"Okay, I won't proceed with {pending.get('pending_role', 'that action')}.",
                    "pending_action": pending.get("pending_action"),
                }
                self._append_chat(user_id, "assistant", response["message"])
                return self._finalize(response, self.SOURCE_KEYWORD, profile=profile)

        # Active search continuation: user acknowledged a search result
        if intent_result.intent in ("smalltalk", "confirmation") and message.lower() in ("ok", "okay", "continue", "go on"):
            active_search = self.memory.get_active_search_context(user_id)
            if active_search:
                return self._handle_active_search_continuation(user_id, active_search, profile)
    ...
    def _classified_role_search(self, user_id: str, role_text: str, profile: Any) -> dict[str, Any]:
        ...
        if classification == "known_but_off_profile" and canonical_role:
            ...
            response = {
                ...
                "pending_action": "confirm_search",
                "pending_role": canonical_role,
                "pending_reason": "known_but_off_profile",
            }
            self._append_chat(user_id, "assistant", response["message"])
            return response
    ...
    def _handle_pending_confirmation(
        self, user_id: str, pending: dict[str, Any], profile: Any
    ) -> dict[str, Any]:
        """Handle user confirmation of a pending action."""
        pending_action = pending.get("pending_action")
        pending_role = pending.get("pending_role")
        pending_reason = pending.get("pending_reason")

        # Clear the pending state
        self.memory.clear_conversation_state(user_id)

        if pending_action == "confirm_search" and pending_role:
            # Execute the pending search
            response = self._target_role_search_response(user_id, pending_role, profile)
            response["confirmed_action"] = pending_action
            response["confirmed_role"] = pending_role
            response["confirmation_reason"] = pending_reason
            return self._finalize(response, self.SOURCE_KEYWORD, profile=profile)

        # Generic confirmation response
        response = {
            "type": "confirmation_accepted",
            "message": f"Confirmed. Proceeding with {pending_action} for {pending_role}.",
            "confirmed_action": pending_action,
            "confirmed_role": pending_role,
            "confirmation_reason": pending_reason,
        }
        self._append_chat(user_id, "assistant", response["message"])
        return self._finalize(response, self.SOURCE_KEYWORD, profile=profile)

    def _handle_active_search_continuation(
        self, user_id: str, active_search: dict[str, Any], profile: Any
    ) -> dict[str, Any]:
        """Handle continuation of an active search context."""
        active_role = active_search.get("active_role")
        last_result_count = active_search.get("last_result_count", 0)
        fallback_roles = active_search.get("fallback_roles", [])
        next_action = active_search.get("next_action", "broaden_search_or_show_profile_roles")

        # Clear the active search state after handling
        self.memory.clear_active_search_context(user_id)

        if last_result_count == 0 and fallback_roles:
            # No results found - offer fallback roles
            response = {
                "type": "search_fallback_options",
                "message": (
                    f"I didn't find exact matches for {active_role}. "
                    f"Based on your profile, you might also consider: {', '.join(fallback_roles[:3])}."
                ),
                ...
                "options": [
                    {
                        "action": "search_fallback_role",
                        "label": f"Search for {fallback_roles[0]}",
                        "message": f"search for {fallback_roles[0]}",
                        "role": fallback_roles[0],
                    }
                    for fallback_roles in fallback_roles[:3]
                ],
            }
            self._append_chat(user_id, "assistant", response["message"])
            return self._finalize(response, self.SOURCE_KEYWORD, profile=profile)
        ...
        response = {
            "type": "search_continuation",
            "message": f"Continuing from your search for {active_role}.",
            "active_role": active_role,
            "last_result_count": last_result_count,
            "next_action": next_action,
        }
